<template>
    <div>
                ...
                <div v-if="pagemessages.length > 0">
                    <el-timeline-item v-for="(item, index) in pagemessages" :key="index" :timestamp="item.createDate"
                        ...
                        <el-card class="el-card-m">
                            ...
                            <span v-if="item.replyContent" class="el-card-m-reply">{{ item.replyName }}回复：{{
        ...
        <div class="el-card-messages">
            ...
            <el-input slot="prepend" v-model="message" type="textarea" :rows="2" class="message-text" placeholder="输入留言"
                maxlength="200" />

            <el-button type="info" round class="submit-message" size="mini" @click="submitMessage">留言</el-button>
        </div>
    </div>
</template>
