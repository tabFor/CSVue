<template>
    <div class="signin">
        <div>
            <el-input placeholder="请输入邮箱" v-model="name" clearable class="input_style2"></el-input>
            <el-tag :type="isValidEmail(this.name) ? '' : 'danger'">{{ namecheck }}</el-tag>
        </div>

        <div>
            <el-input placeholder="验证码" v-model="pwd" class="input_style2"></el-input>
            <el-button type="primary" @click="getcode" v-loading="loading"
                :disabled="!(isValidEmail(this.name) && !laoding)">发送</el-button>
        </div>
        <div>
            <el-button :disabled="!(isValidEmail(this.name) && (this.pwd.length === 6) && !laoding)" type="primary"
